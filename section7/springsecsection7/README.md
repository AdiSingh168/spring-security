...
- Added condition to accept only https requests for Prod environment, and accept only http requests for Non-Prod
  environments.
- Added custom handlers for exception handling: CustomAccessDeniedHandler and CustomBasicAuthenticationEntryPoint.
- Added session timeout logic.
- Added invalid session configurations.
- Added Maximum concurrent sessions logic.